...
from puzzlesolver.Puzzle.Constraints import OutsideEdgeConstraint
...
def genConstraint(puzzle: Puzzle, tool_key: OutsideEdgeConstraintsE):
    ...
    for constraint in constraints_list:
        if not isinstance(constraint, OutsideEdgeConstraint):
            continue
        yield constraint


def genOutsideEdgeConstraintProperties(model: PuzzleModel, puzzle: Puzzle, tool_key: OutsideEdgeConstraintsE):
    tool_constraints = puzzle.tool_constraints
    constraints_list = tool_constraints.get(
        tool_key)
    if not constraints_list:
