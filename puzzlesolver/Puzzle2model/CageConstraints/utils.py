...
from puzzlesolver.Puzzle.Constraints import CageConstraint
...
def genConstraint(puzzle: Puzzle, tool_key: CageConstraintsE):
    ...
    for constraint in constraints_list:
        if not isinstance(constraint, CageConstraint):
            continue
        yield constraint


def genCageConstraintProperties(model: PuzzleModel, puzzle: Puzzle, tool_key: CageConstraintsE):
    tool_constraints = puzzle.tool_constraints
    constraints_list = tool_constraints.get(
        tool_key)
    if not constraints_list:
